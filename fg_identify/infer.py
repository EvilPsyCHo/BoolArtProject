...
def get_prediction(similarity_df, method='simple', N=100):
    ...
    for q in predictions.keys():
        matched_1[q] = predictions[q]['similarity'][0]
        matched_3[q] = (np.mean(predictions[q]['similarity'][:3]) + matched_1[q]) / 2
        matched_10[q] = (np.mean(predictions[q]['similarity'][:10]) + matched_1[q] + matched_3[q]) / 3
